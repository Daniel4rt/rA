...
void channel_join(struct map_session_data *sd, const char* name, const char* pass, bool invite)
{
	...
	if( !invite )
	{
		...
		{
			clif_displaymessage(sd->fd, msg_txt(sd,703));
			return;
		}
	}
	...
	sprintf(output, msg_txt(sd,710), sd->status.name, cd->name);
...
int channel_invite_timer(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	{
		sprintf(output, msg_txt(sd,701), cd->name, sd->status.name);
		clif_channel_message(cd, output, -1);
	}

	if( sd )
	{
		clif_disp_onlyself(sd, msg_txt(sd,702), strlen(msg_txt(sd,702)));
	...
	{ // Channel commands
		...
		{ // Invite a User to the Channel
			...
			if( (p_sd = map_nick2sd(option_text)) == NULL )
				...
				clif_displaymessage(sd->fd, msg_txt(sd,700));
